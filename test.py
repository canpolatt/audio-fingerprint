...
if __name__ == '__main__':
    ...
    reader = FileReader("Mozart - Lacrimosa(10seconds).mp3")
    ...
    def return_matches(hashes):
        ...
        for split_values in grouper(values, 1000):
            x = db.executeAll('SELECT upper(hash_value), song_fk, offset_value FROM fingerprints WHERE upper(hash_value) IN %s', tuple(split_values))
            matches_found = len(x)
